    ...
    *coord_fix* is an optional Matrix applied to the raw EDM AABB corners before
    ...
    try:
        ob["_iedm_raw_bbox"] = json.dumps(
            [
                float(root.boundingBoxMin[0]),
                float(root.boundingBoxMin[1]),
                float(root.boundingBoxMin[2]),
                float(root.boundingBoxMax[0]),
                float(root.boundingBoxMax[1]),
                float(root.boundingBoxMax[2]),
            ],
            separators=(",", ":"),
        )
...
def _has_special_box(special_type):
    ...
    for obj in bpy.data.objects:
        if (
            ...
            and getattr(obj.EDMProps, "SPECIAL_TYPE", "") == special_type
        ):
            return True
    return False
...
def _cache_root_aabb_payloads_on_scene(root):
    ...
    try:
        ...
        if user_box is not None:
            ...
            if not any(math.isinf(v) for v in min_vec_edm) and not any(
                math.isinf(v) for v in max_vec_edm
            ):
...
def _create_special_box(
    name, special_type, min_vec_edm, max_vec_edm, raw_prop_name, coord_fix=None
):
    ...
    dims = Vector(
        (
            abs(max_vec.x - min_vec.x),
            abs(max_vec.y - min_vec.y),
...
def _create_user_box_from_root(root, coord_fix=None):
    ...
    min_vec_edm = Vector(user_box[0])
    max_vec_edm = Vector(user_box[1])
    if any(math.isinf(v) for v in min_vec_edm) or any(
        math.isinf(v) for v in max_vec_edm
    ):
        return None
    return _create_special_box(
        "User_Box",
        "USER_BOX",
        min_vec_edm,
        max_vec_edm,
