...
def inspect_import_graph(edm):
    ...
    has_owner_encoded_split_renders = any(
        getattr(obj, "shared_parent", None) is not None
        for obj in _iter_all_objects(edm)
    )
    has_generic_render_chunks = any(
        isinstance(obj, RenderNode)
        and str(getattr(obj, "name", "") or "").startswith("_")
        for obj in edm.renderNodes
    )
    has_shell_nodes = bool(edm.shellNodes)
    has_segments_nodes = any(
        isinstance(obj, SegmentsNode) for obj in _iter_all_objects(edm)
    )
...
def derive_import_capabilities(edm):
    ...
    flags = {}
    authored_scene_root_v10 = bool(
        features.edm_version >= 10
        and features.plain_root_v10
        and features.root_name_is_scene_root
    )
...
def _capability_profile(features, authored_scene_root_v10, pure_collision_payload):
    ...
    if pure_collision_payload:
        ...
        description = (
            "Embedded collision-style graph derived from parsed shell/segment nodes."
        )
