...
for filename in sys.argv[sys.argv.index("--") + 1 :]:
    ...
    for mode in ("RAW", "APPROX"):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            reader.read_file(
                str(path),
                options={"mesh_origin_mode": mode, "preserve_scene_boxes": True},
            )
        for frame in (100, 0, 150, 200):
            ...
            if mode == "APPROX":
                assert set(meshes) == set(reference[frame]), (
                    path.name,
                    "mesh names changed",
                )
            else:
            ...
            for name, obj in meshes.items():
                ...
                indices = (
                    range(len(vertices))
                    if frame == 100
                    else range(0, len(vertices), max(1, len(vertices) // 32))
                )
                ...
                assert len(points) == len(expected) and state == visibility, (
                    path.name,
                    name,
                    frame,
                )
                error = max(
                    ((p - q).length for p, q in zip(points, expected, strict=False)),
                    default=0,
                )
