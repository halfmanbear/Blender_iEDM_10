...
for path in [Path(sys.argv[-1]).resolve()]:
    ...
    for owner in list(bpy.data.objects) + list(bpy.data.lights) + list(bpy.data.node_groups):
    ...
    groups = sum(1 for mat in bpy.data.materials if mat.node_tree for node in mat.node_tree.nodes if getattr(node, 'node_tree', None) is not None)
    assert groups > 0, 'No official material groups created'
    for mat in bpy.data.materials:
        if mat.node_tree:
            for node in mat.node_tree.nodes:
                tree = getattr(node, 'node_tree', None)
                desc = (_ensure_official_material_bridge()['material_descs'].get(tree.name)
                        if tree is not None else None)
                # Some exporter material kinds are interface-only placeholders.
                if desc is not None and desc.nodes:
                    assert len(tree.nodes) >= len(desc.nodes), (tree.name, len(tree.nodes), len(desc.nodes))
                    if desc.links:
                        assert len(tree.links) > 0, (tree.name, len(desc.links))
    print('INTEGRATION_PASS', path.name, 'objects', len(bpy.data.objects), 'shader_groups', groups)
    iEDM_10.unregister()
